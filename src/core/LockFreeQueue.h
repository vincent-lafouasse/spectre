...
// project specific
//
// check that a suitable size type is always lock free and define it
...
struct LockFreeQueue {
    // avoid false sharing
    alignas(CACHE_LINE) _Atomic(SizeType) front;
    char pad1[CACHE_LINE - sizeof(_Atomic(SizeType))];

    alignas(CACHE_LINE) _Atomic(SizeType) back;
    char pad2[CACHE_LINE - sizeof(_Atomic(SizeType))];
...
struct LockFreeQueueProducer {
    _Atomic(SizeType)* back;         // sole writer
    const _Atomic(SizeType)* front;  // read only
    SizeType cached_front;           // avoid pessimistic loads
    float* data;
};
...
struct LockFreeQueueConsumer {
    _Atomic(SizeType)* front;
    const _Atomic(SizeType)* back;
    SizeType cached_back;
    const float* data;
};
